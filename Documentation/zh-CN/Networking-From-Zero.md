...
所有网关模式客户端的接线工作都由一个函数 **从宿主机侧**完成，即 `gateway_wire_client()`：它确保网桥和网关侧线缆就位，创建客户端的应用 veth，并将对端移入并重命名到客户端命名空间中作为 `eth0`（固定 MAC，已启用）——客户端自身的启动代码仅启用 `lo`。由于宿主机掌控每一步，同一个函数既可以为刚开始启动的客户端接线，也可以为已经在运行中的客户端接线。

这产生了一条基于**网关存活状态**的简单规则：

- **客户端启动时网关已在运行** → 客户端立即完成接线（其自身的监视器调用 `gateway_wire_client`）。
- **客户端启动时网关未运行** → 客户端**完全不进行任何接线**（无网桥、无 veth、无 `eth0`），直接启动。工作完全推迟给网关。

因此，自愈由**网关自身**驱动，而非由客户端驱动。在每个启动周期中，网关容器的监视器会调用 `ds_net_rewire_gateway_clients()`：扫描正在运行的容器，找到委托给该网关的容器，并对每个容器运行 `gateway_wire_client`——在网关的**当前**命名空间中建立网关侧的 `eth1` 线缆和每个客户端的 `eth0`。因此，当网关**启动或重启**时，每个运行中的客户端都会被重新接线，**无需重启客户端**。

在网关下线期间完全跳过客户端接线（而不是半接线一个网桥和一个悬空的 veth），同时也填补了一个竞态条件：在网关启动之前，客户端可以使用网关/LAN 设置（`--gateway-net`、`--host-bridge` 等）进行编辑，而网关随后会从每个客户端的*当前*配置进行接线——绝不会使用过时配置。

接线工作有且仅有 **一个执行者**（网关），因此无需轮询，也没有惊群效应。接线按网段序列化（通过一个文件建议锁），防止并发客户端启动与网关的重新接线产生竞态。`eth1`（网关侧）和每个 `eth0`（客户端侧）都保持**稳定的 MAC 地址**，并在单个原子步骤中移入并重命名到各自的命名空间中，因此容器自身的 `netifd`/DHCP 看到的是一个持久的设备，而不是反复变更的设备。
...
- **最后一个客户端停止时网关仍在运行：** 网桥**被保留**（不会被回收）。拆除网桥会导致网关活跃的 `eth1` 链路发生载波闪断，偶尔会使 netifd 报告"device initialization failed"；一个空闲的无 IP 网桥是无害的，下一个客户端可以重用。
- **网关停止：** 网关侧的 veth 随其命名空间一起消失。一旦没有客户端并且网关也已退出，则空闲的网桥被回收。
